                    ...
                    <div class="content">
                        <div class="signin-cont cont">
                            <form method="POST" action="{{ route('login') }}">
                                @csrf
                                <input type="email" name="email" id="email" class="inpt" required="required" placeholder="Your email" value="{{ old('email') }}" autocomplete="email" autofocus>
                                ...
                                @enderror
                                <input type="password" name="password" id="password" class="inpt" required="required" placeholder="Your password" @error('password') is-invalid @enderror autocomplete="current-password">
                        ...
                        <div class="signup-cont cont">
                            <form action="{{ route('register') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="text" name="name" id="name" class="inpt" @error('name') is-invalid @enderror required="required" value="{{ old('name') }}" placeholder="Your name">
                                ...
                                @enderror
                                <input type="email" name="email" id="email" class="inpt" @error('email') is-invalid @enderror required="required" value="{{ old('email') }}" placeholder="Your email">
                                <label for="email">Your email</label>
                                @error('email')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                ...
                                @enderror
                                <input type="password" name="password" id="password" class="inpt" @error('password') is-invalid @enderror required="required" placeholder="Your password">
                                <label for="password">Your password</label>
                                @error('password')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                ...
                                @enderror
                                <input type="password" name="password_confirmation" id="password-confirm" class="inpt" required="required" placeholder="Confirm password" required autocomplete="new-password">
    ...
    <div class="self-trip">
        ...
        <div class="orders-detail">
          <div class="table-responsive">
              <table class="table table-sm table-Light table-striped table-hover align-middle caption-top">
                  <caption class="lead">目前已建立的自訂行程</caption>
                  <thead>
                      <tr>
                          <th style="width: 25%; min-width: 100px;">建立日期</th>
                          <th>行程名稱</th>
                          <th style="width: 10%; min-width: 56px;">操作</th>
                      </tr>
                  </thead>
                  <tbody>
                    @foreach ($orders as $order)
                    <tr>
                        <td scope="row">{{ Str::substr($order->created_at, 0, 16) }}</td>
                        <td>{{ $order->name }}</td>
                        <td><button type="button" class="btn btn-primary btn-sm order-btn" 
                          data-createdTime="{{ Str::substr($order->created_at, 0, 16) }}" 
                          data-orderName="{{ $order->name }}" 
                          data-spotsData="{{ json_encode($order->spots) }}">詳細</button></td>
                    </tr>
                    @endforeach
                  </tbody>
              </table>
