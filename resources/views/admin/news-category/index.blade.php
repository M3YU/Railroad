...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                <h2 class="card-header lh-1 pt-3 pb-2">新聞類別管理</h2>
                <div class="form-group pt-4 px-3 m-0">
                    <a href="{{route('news_categories.create')}}" class="btn btn-success">新增新聞類別</a>
                </div>
                <hr>
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($newsCategories as $newsCategory)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('news_categories.edit', ['news_category' => $newsCategory])}}" class="btn btn-primary">編輯</a>
                                        <button class="btn btn-danger delete-btn">刪除</button>
                                        <form class="d-none" action="{{route('news_categories.destroy', ['news_category' => $newsCategory])}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
